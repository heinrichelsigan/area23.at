...
{
    public partial class OctalDump : System.Web.UI.Page
    {
        char radix = 'n';
        int hexWidth = 8;
        int wordWidth = 32;
        ...
        /// Perform_Hexdump()
        /// </summary>
        protected void Perform_OctalDump()
        {
            try
            {
                TextBox_OdCmd.Text = odCmdPath + OdArgs;
                preOut.InnerText = Process_OctalDump(odSuidCmdPath, OdArgs);
            } 
            catch (Exception ex)
            {
        ...
        protected string Process_OctalDump(
        ...
        {
            if (OctalDump.random == null)
                OctalDump.random = new Random((int)(DateTime.Now.Ticks % Int32.MaxValue));
