...
namespace Area23.At.Mono.Unix
{
    ...
    /// WhoisDns query page
    /// </summary>
    public partial class HostWhois : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                try
                {
                    IPAddress ip = (Constants.FortuneBool) ? IPAddress.Parse("192.0.47.59") : IPAddress.Parse("192.0.32.59");
                    cmdOut = Sender.Send(ip, Sanitize_HostName(this.TextBox_HostName.Text), 43);
                }
                catch (Exception ex)
                {
        ...
        {            
            ...
            try 
            {
                ...
                args = ((Constants.UNIX) ? "" : " -accepteula ") + args;
                cmdOut = ProcessCmd.ExecuteCreateWindow(filepath, args);
                if (cmdOut.Contains("-accepteula"))
                {
                    Thread.Sleep(100);
                    cmdOut = ProcessCmd.ExecuteCreateWindow(filepath, args);
                }                
        ...
        internal void FormatWhoisLines(string cmdOut)
        {
