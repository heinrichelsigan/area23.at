...
{
    internal class SaySpeachCore
    {
        private static string SepChar { get => Path.DirectorySeparatorChar.ToString(); }
        private static string[] TMPS = { "temp", "TEMP", "tmp", "TMP" };
        protected internal static readonly Lazy<SaySpeachCore> lazySaySingleton = new Lazy<SaySpeachCore>(() => new SaySpeachCore());
        protected internal SpeechSynthesizer synthesizer;
        private static string saveDir;
        protected internal string[] voices;

        internal static SaySpeachCore Instance { get => lazySaySingleton.Value; }
        ...
        protected internal static string[] AppPaths
        {
            ...
            {                
                ...
                if (Application.CommonAppDataPath != null && Application.CommonAppDataPath.Length > 1 && Directory.Exists(Application.CommonAppDataPath))
                    appPathList.Add(Application.CommonAppDataPath);

                if (Application.ExecutablePath != null && Application.ExecutablePath.Length > 1)
                {
                    tmpDir = Path.GetDirectoryName(Application.ExecutablePath);
                    if (tmpDir != null && Directory.Exists(tmpDir)) 
                        appPathList.Add(tmpDir); 
                }
        ...
        protected internal SaySpeachCore()
        {
