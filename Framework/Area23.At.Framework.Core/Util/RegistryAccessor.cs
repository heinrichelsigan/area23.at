...
namespace Area23.At.Framework.Core.Util
{
    ...
    public class RegistryAccessor
    {
        ...
        static RegistryAccessor()
        {
            ...
            _subKey = null;
            _threadCnt = 0;
            TimeSpan oneDay = new TimeSpan(1, 0, 0, 0, 0, 0);
        }
        ...
        public static object? GetRegistryEntry(RegistryHive regHive, string subKeyName, string regName)
        {
            ...
            if (string.IsNullOrEmpty(subKeyName) || string.IsNullOrEmpty(regName))
                throw new CqrException($"subkey {subKeyName} or regName {regName} is null or empty");
            ...
            {
                Area23Log.LogOriginMsgEx("RegistryAccessor",
                    $"GetRegistryEntry operation in regHive {regHive} subKeyName {subKeyName} regName {regName} throwed Exception ", ex);
                throw;
            }
            finally
            {
        ...
        internal static bool DeleteSubTree(RegistryHive regHive, string subTree)
        {
            ...
            if (string.IsNullOrEmpty(subTree))
                throw new CqrException($"subkey {subTree} is null or empty");

            try
            {
                Area23Log.LogOriginMsg("RegistryAccessor",
                    $"creating mutex {Constants.MUTEX_REGOPS} for delete operation in regHive {regHive} subTree {subTree}");
                if ((_regOpMutex = new Mutex(true, Constants.MUTEX_REGOPS)) != null)
                {
                    while (_regOpMutex != null && !_regOpMutex.GetSafeWaitHandle().IsClosed && !
                        _regOpMutex.GetSafeWaitHandle().IsInvalid && !_regOpMutex.WaitOne(64, false))
