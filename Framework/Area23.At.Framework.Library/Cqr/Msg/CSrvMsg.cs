    ...
    public class CSrvMsg<TC> : CContent, IMsgAble where TC : class
    {
            ...
            protected internal set
            {
                if (value != null && !string.IsNullOrEmpty(value.NameEmail))
                {
                    ...
                    else
                    {
                        ...
                        {
                            if ((value.Cuid != Guid.Empty && value.Cuid == Recipients.ElementAt(ix).Cuid) &&
        ...
        public CSrvMsg(string serializedString, SerType msgArt = SerType.Json) : base()
        {
            ...
            if (deserializedSrvMsg == null)
                throw new CqrException($"Cannot deserialize serializedString={serializedString}, SerType={msgArt} to CSrvMsg<TC>.");
            ...
            Zfx.ZipType zipType = Zfx.ZipType.None, KeyHash kHash = KeyHash.Hex)
        {
            ...
            try
            {
                pipeString = (new CipherPipe(serverKey, keyHash)).PipeString;

                decrypted = CipherPipe.DecrpytT<string, string>(Message, serverKey, keyHash, decoder, zipType, kHash);
             ...
             EncodingType decoder = EncodingType.Base64, Zfx.ZipType zipType = Zfx.ZipType.None)
        {
            ...
            try
            {
                ...
                string decrypted = CipherPipe.DecrpytT<string, string>(cSrvMsg.Message, serverKey, keyHash, 
                    decoder, zipType, KeyHash.Hex);                    
