...
namespace Area23.At.Framework.Library.Core.Win32Api
{
    ...
    public static class Processes
    {
        ...
        public static bool ProcessRuns(int pid)
        {
            ...
            {
                process = null;
                Library.Core.Area23Log.Logger.LogOriginMsgEx($"bool Processes.ProcessRuns(int pid = {pid})",
        ...
        public static Process GetProcessById(int pid)
        {
            ...
            {
                process = null;
                Library.Core.Area23Log.Logger.LogOriginMsgEx($"Processes.GetProcessById(int pid = {pid})",
                    $"Error on fetching process for pid {pid}.", exProc);
            }
        ...
        public static Process GetProcessByHwnd(IntPtr hwnd)
        {
            ...
            {
                Library.Core.Area23Log.Logger.LogOriginMsgEx($"Processes.GetProcessByHwnd(IntPtr hwnd = {hwnd})",
        ...
        public static Process[] GetRunningProcessesByName(string processName)
        {
            ...
            foreach (Process p in Process.GetProcessesByName(processName))
            {
                ...
                {
                    Library.Core.Area23Log.Logger.LogOriginMsgEx($"Processes.GetRunningProcessesByName(string processName = {processName}, string windowTitle = {p.MainWindowTitle})",
        ...
        public static Process[] GetProcessByNameAndWindowTitle(string processName, string windowTitle)
        {
            ...
            foreach (Process p in Process.GetProcessesByName(processName))
            {
                ...
                {
                    Library.Core.Area23Log.Logger.LogOriginMsgEx($"Processes.GetRunningProcessesByName(string processName = {processName}, string windowTitle = {windowTitle})",
        ...
        public static Dictionary<int, string> GetChildIdNames(int pPid, bool addParent = false)
        {
            ...
            {
                Library.Core.Area23Log.Logger.LogOriginMsgEx(methodSignature, $"Exception in getting process with id [{pPid}].", getProcEx);
                return parentChildProcessDict;
            }

            if (parentProcess == null || parentProcess.HasExited)
            {
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Process with id [{pPid}] is null or has already exited.");
                return parentChildProcessDict;
            }
            ...
            if (addParent)
            {
                ...
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Added parent process <[pPid = {pPid}], [ppName = {parentProcess.ProcessName}]> to  Dictionary<int, string>().");
            }
            ...
            foreach (var elem in processList)
            {
                try
                {
                    ...
                    if (childProcess != null && !childProcess.HasExited)
                    {
                        ...
                        Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Added child process <[cPid = {elem.Key}], [cpName = {childProcess.ProcessName}]> to  Dictionary<int, string>().");
                    }
                }
                catch (Exception getChildProcEx)
                {
                    Library.Core.Area23Log.Logger.LogOriginMsgEx(methodSignature, $"Exception in getting child process with id [{elem.Key}].", getChildProcEx);
                }
            }
        ...
        public static Process[] ListChildProcesses(int pPid, bool addParent = false)
        {
            ...
            if (addParent) // add parent to HashSet  
            {
                ...
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Added parent process \"{parentProcess.ProcessName}\" with id [{pPid}] to HashSet<int> parentChildProcessHash.");
            }
            ...
            foreach (var elem in processHandleList)
            {
                try
                {
                    ...
                    if (childProcess != null && !childProcess.HasExited && !parentChildProcessHash.Contains(childProcess))
                    {
                        ...
                        Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Added child process[Id = {elem.Key}, ProcesName = {childProcess.ProcessName}, ...] to HashSet<int> parentChildProcessHash[{parentChildProcessHash.Count}].");
                    }
                }
                catch (Exception getChildProcEx)
                {
        ...
        public static void KillProcess(int pid, bool ignoreWin32SystemProcesses = true)
        {
            ...
            {
                Library.Core.Area23Log.Logger.LogOriginMsgEx(methodSignature, $"Exception in getting process with id [{pid}].", exGetProcess);
                return;
            }

            if (process == null || process.HasExited)
            {
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Aborting tree recursion, because process with id [{pid}] is null or has already exited.");
                return;
            }

            if (ignoreWin32SystemProcesses && string.IsNullOrEmpty(process.ProcessName))
            {
                foreach (string winSystemProcName in Library.Core.Constants.EXE_WIN_SYSTEM)
                {
                    ...
                    {
                        Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Ignoring win32 system process \"{process.ProcessName}\" with id [{pid}]  window title '{process.MainWindowTitle}'.");
                        return;
                    }
                }
            }
            ...
            try
            {
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Trying to kill process \"{process.ProcessName}\" with id [{pid}].");
                process.Kill();
            }
            catch (Exception killProcEx)
            {
                Library.Core.Area23Log.Logger.LogOriginMsgEx(methodSignature, $"Exception in killing process \"{process.ProcessName}\" with id [{pid}].", killProcEx);
            }
            ...
            if (process != null)
            {
                ...
                {
                    Library.Core.Area23Log.Logger.LogOriginMsgEx(methodSignature, $"Exception in diposing process with id [{pid}].", disposeProcEx);
                }
            }
        }
        ...
        public static int KillProcessTree(int pid, bool killParent = true, int psKilled = 0, bool ignoreWin32SystemProcesses = true)
        {
            ...
            if (ignoreWin32SystemProcesses && string.IsNullOrEmpty(processName))
            {
                foreach (string winSystemProcName in Library.Core.Constants.EXE_WIN_SYSTEM)
                {
                    ...
                    {
                        Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Ignoring win32 system process \"{processName}\" with id [{pid}]  window title '{process.MainWindowTitle}'.");
                        return psKilled;
                    }
                }
            }
            ...
            {
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Found {childs.Length} child processes for process \"{processName}\" with id [{pid}] window title '{process.MainWindowTitle}'.");
            ...
            if (killParent)
            {
                try
                {
                    Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Trying to kill process \"{processName}\" with id [{pid}].");
                    process.Kill();
                    psKilled++;
                }
                catch (Exception killProcEx)
                {
                    Library.Core.Area23Log.Logger.LogOriginMsgEx(methodSignature, $"Exception in killing process \"{processName}\" with id [{pid}].", killProcEx);
                }
            }
            else
            {
                Library.Core.Area23Log.Logger.LogOriginMsg(methodSignature, $"Ignoring killing parent process \"{processName}\" with id [{pid}].");
            }
